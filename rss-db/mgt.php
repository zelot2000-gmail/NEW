...
if($rss->pubDate) {
	...
	if($_GET["link_id"]!="") {
		...
		$SQL = "update tb_rss set confirm='0' where link_id='".$_GET["link_id"]."' and deptid='".$deptid."'  ";
		mysqli_query($mysqli, $SQL) or die(mysqli_error($mysqli).$SQL);
	}
?>
<!doctype html>
			...
			<div class="card">
			  ...
			  <div class="card-header">
					<div class="row">
						...
						<div class="col-sm-5">
							<select class="form-control" name="type" id="type" onchange="go_ajax('mgt', this.value, '<?php print $deptid?>')">
...
//					Table.rows.add(msg).draw();
		...
		go_ajax('mgt', 'P0', '<?php print $deptid?>' );


		function del() {
			if(confirm("คุณต้องการลบรายการนี้ออกหรือไม่?")) {
				return true;
			} else {
				return false;
			}
